...
def generate_past_month_data(user_id):
    ...
    for i in range(num_transactions):
        ...
        pattern = next(p for p in expense_patterns if p['category'] == category)
        ...
        if remaining_transactions > 0:
            ...
            if avg_remaining > max_amt:
                ...
                amount = random.randint(min(min_amt, remaining_amount), remaining_amount)
            else:
...
def get_stats():
    ...
    user_id = session['user_id']
    user_expenses = expenses.get(user_id, [])
    user = next(u for u in users.values() if u['id'] == user_id)
